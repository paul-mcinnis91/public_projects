...
class DownloadFilesfromOBI:
    ...
    def OBI_Login(self):
        ...
        user_ID = get_user_creds().get("obi_username")
        password = get_user_creds().get("obi_password")
    ...
    def download_files_mon(self, attachment_rate_urls: dict, driver: webdriver.Firefox) -> None:
        if self.weekday != 0:
            raise TypeError("Wrong Day of Week")
    ...
    def failed_thurs_download(self, current_position: int, reference_data_urls: dict, driver: webdriver.Firefox) -> None:
    ...
    def download_files_thurs(self, reference_data_urls: dict, driver: webdriver.Firefox) -> None:
        try:
            ...
            time.sleep(600)
            self.failed_thurs_download(current_position=current_position, reference_data_urls=reference_data_urls, driver=driver)
    ...
    def download_files(self):
        ...
        if day_of_week == 0:
            self.download_files_mon(attachment_rate_urls=attachment_rate_urls, driver=driver)
            self.download_files_thurs(reference_data_urls=reference_data_urls, driver=driver)
            driver.close()
            driver.quit()
            return None
